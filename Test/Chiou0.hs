...
loadTest :: Test
loadTest =
    TestCase (assertEqual "Chiuo0 - loadKB test" expected (runSkolem (runProverT (loadKB sentences))))
    where
...
proofTest1 :: Test
proofTest1 = TestCase (assertEqual "Chiuo0 - proof test 1" proof1 (runSkolem (runProverT (loadKB sentences >> theoremKB (Predicate "Kills" [Function (Fn "Jack") [], Function (Fn "Tuna") []])))))
...
proofTest2 :: Test
proofTest2 = TestCase (assertEqual "Chiuo0 - proof test 2" proof2 (runSkolem (runProverT (loadKB sentences >> theoremKB (Predicate "Kills" [Function (Fn "Curiosity") [], Function (Fn "Tuna") []])))))
