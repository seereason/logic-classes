    ...
    foldFirstOrder
          ...
          (\ cm ->
               case cm of
                 ...
                 TRUE -> S.empty
                 FALSE -> S.empty)
          (foldAtomEq (\ _ ts -> S.unions (fmap freeVarsOfTerm ts)) (\ t1 t2 -> S.union (freeVarsOfTerm t1) (freeVarsOfTerm t2)))
          f
    where
    ...
    foldFirstOrder
          ...
          (\ cm ->
               case cm of
                 ...
                 TRUE -> S.empty
                 FALSE -> S.empty)
          (foldAtomEq (\ _ ts -> S.unions (fmap allVarsOfTerm ts)) (\ t1 t2 -> S.union (allVarsOfTerm t1) (allVarsOfTerm t2)))
          f
    where
      ...
      substitute' =
          foldFirstOrder -- If the old variable appears in a quantifier
                ...
                (\ cm -> case cm of
                           ...
                           TRUE -> true
                           FALSE -> false)
                (foldAtomEq (\ p ts -> pApp p (map st ts)) (\ t1 t2 -> (st t1) .=. (st t2)))
...
convertFOFEq convertV convertP convertF formula =
    ...
    where
      ...
      c TRUE = true
      c FALSE = false
      p = foldAtomEq (\ x ts -> pApp (convertP x) (map convertTerm' ts)) (\ t1 t2 -> (convertTerm' t1) .=. (convertTerm' t2))
...
fromFirstOrderEq cv cp cf formula =
    ...
    where
      ...
      p :: atom -> lit
      p = foldAtomEq (\ pr ts -> Data.Logic.Classes.Literal.atomic (applyEq (cp pr) (map ct ts))) (\ a b -> Data.Logic.Classes.Literal.atomic (ct a `equals` ct b))
