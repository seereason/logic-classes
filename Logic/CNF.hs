...
cnf :: (Eq formula, Eq term, Formulae formula term v p f, Show formula) => formula -> formula
...
skolemize :: (Eq formula, Eq term, Formulae formula term v p f) => [v] -> formula -> formula
skolemize uq =
    foldF n q b i a
    where
      n x = (.~.) (skolemize uq x)
...
skolemFunction :: (IsString f, Formulae formula term v p f) => [v] -> v -> term
...
simplify :: Formulae formula term v p f => formula -> formula
simplify =
    foldF n q b i a
    where
      q All vs f = for_all vs (simplify f)
      q Exists vs f = exists vs (simplify f)
      b :: Formulae formula term v p f => formula -> BinOp -> formula -> formula
...
moveNegationsIn :: Formulae formula term v p f => formula -> formula
moveNegationsIn =
    foldF n q b i a
    where
      n f = doNegation f
      ...
      doNegation :: Formulae formula term v p f => formula -> formula
      doNegation =
          foldF n' q' b' i' a'
          where
...
moveQuantifiersOut :: (Formulae formula term v p f, Show formula) => formula -> formula
moveQuantifiersOut formula =
    ...
    where
      ...
      doLHS :: (Formulae formula term v p f, Show formula) => formula -> BinOp -> formula -> formula
      doLHS f1 op f2 =
          foldF n' q' b' i' a' f1
          where
            n' _ = doRHS f1 op f2
      ...
      doRHS :: (Formulae formula term v p f, Show formula) => formula -> BinOp -> formula -> formula
      doRHS f1 op f2 =
          foldF n' q' b' i' a' f2
          where
      ...
      doBinOp :: (Formulae formula term v p f, Show formula) => formula -> BinOp -> formula -> formula
      doBinOp f1 (:&:) f2 = f1 .&. f2
      doBinOp f1 (:|:) f2 = f1 .|. f2
...
-- a|(b&c) -> (a|b)&(a|c)
distributeDisjuncts :: (Eq formula, Formulae formula term v p f) => formula -> formula
distributeDisjuncts =
    foldF n q b i a
    where
      n = (.~.)
